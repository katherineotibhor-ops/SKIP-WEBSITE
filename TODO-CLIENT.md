...
| 23 | ~~**Branded email address**~~ **Done in code — mailbox must exist** | Client | All 37 references to `Elevatedid1@gmail.com` across 11 site files have been replaced with `consultations@elevatedidentities.com` — footer on all 10 pages, the Contact page details and meta descriptions, the enquiry-form fallback messages in `assets/js/main.js`, and the contact points in the Privacy Policy and Terms. **The address must now actually receive mail before launch.** That needs a mailbox on the domain (Namecheap Private Email, Google Workspace, or similar) and the matching MX records added in Namecheap DNS. Until then every address on the site bounces, which is worse than the Gmail address was. Note this address is also the data-rights contact in the Privacy Policy and the notice address in the Terms, so it must be monitored, not just deliverable. |
| 24 | ~~**Verify the LinkedIn URL**~~ **Done** | Client | The footer previously linked to `linkedin.com/in/elevated-id-undefined-470a32432`, a personal profile whose slug contained "undefined". All 10 pages now link to the company page, `https://www.linkedin.com/company/elevated-identities/`, supplied by the client. The link's accessible label ("Elevated Identities on LinkedIn") was already correct and is unchanged. |
| 24b | **Clean the Facebook and Instagram URLs** | Developer | Both footer links are "copy link from the app" URLs carrying session tracking parameters: `instagram.com/_elevatedid?igsi=…` and `facebook.com/share/19DXJfipss/?mibextid=…`. They work, but they are not canonical. The Instagram one is safely cleaned to `https://www.instagram.com/_elevatedid`. The Facebook one is a `/share/` link rather than a page URL — if the page has a username, the clean form is `facebook.com/<username>`. Left as supplied rather than changed without confirmation. |
| 25 | ~~**Confirm the business phone number**~~ **Done** | Client | The client supplied `+1 904 906 2310`, which replaces `(602) 824-8796` in all 34 places it appeared: the footer on all 10 pages, the call buttons on Home, Services and FAQ, the Contact page details and meta descriptions, the About and FAQ body copy, the Privacy and Terms contact points, the enquiry-form fallback message in `assets/js/main.js`, and the social share image (re-exported). Displayed as `(904) 906-2310` to match the site's existing US format; `tel:` links use `+19049062310`. Note 904 is a Jacksonville, Florida area code while the site names South Carolina as headquarters; supplied by the client as the business line, so no action needed unless the headquarters display should change too (see item 6). |
