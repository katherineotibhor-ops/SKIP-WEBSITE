...
| 2 | **Confirm the domain** | Client | Not owned/confirmed. `https://www.elevatedidentities.com` is a placeholder in the canonical tags, Open Graph tags, `sitemap.xml` and `robots.txt`. README has the search-and-replace command. |
| 3 | **Export the social share image as PNG** | Developer | `assets/img/og-image.svg` is built to spec as an SVG. **Facebook, LinkedIn and X do not render SVG `og:image`** — the share card will fall back to a blank preview. Export at 1200×630 to `assets/img/og-image.png` and change the four `og:image` / `twitter:image` tags in each page's `<head>`. Flagged rather than silently substituted, since the brief specified SVG. |
...
| 10 | **Testimonials** | Client | No reviews were supplied, and none have been invented. `testimonials.html` currently explains that reviews are being collected. Supply reviews **with written client permission to publish**, and the page will be populated — the markup template is in an HTML comment in that file. |
